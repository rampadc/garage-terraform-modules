...
resource "null_resource" "jenkins_release_openshift" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/scripts/deploy-jenkins-openshift.sh ${self.triggers.tools_namespace} ${var.server_url}"
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "${path.module}/scripts/destroy-jenkins.sh ${self.triggers.tools_namespace}"
  }
}
